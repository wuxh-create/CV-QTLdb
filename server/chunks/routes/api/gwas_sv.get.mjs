import { d as defineEventHandler, g as getQuery, e as GwasSV } from '../../nitro/nitro.mjs';
import 'mongoose';
import 'node:http';
import 'node:https';
import 'node:events';
...
const gwas_sv_get = defineEventHandler(async (event) => {
  const {
    // variant_type,
    variant_id,
    indicator_type,
  ...
  const [total, raw_result_cursor, indicator_type_stats] = await Promise.all([
    GwasSV.countDocuments(query),
    pageSizeNumber === -1 ? GwasSV.find(query).sort(sortBy ? [[sortBy, Number(order)]] : []) : GwasSV.find(query).sort(sortBy ? [[sortBy, Number(order)]] : []).skip(skip).limit(limit),
    // GwasSV.aggregate([
    //     { $match: query },
...
export { gwas_sv_get as default };
//# sourceMappingURL=gwas_sv.get.mjs.map
