import { d as defineEventHandler, g as getQuery, a as GwasMNV } from '../../nitro/nitro.mjs';
import 'mongoose';
import 'node:http';
import 'node:https';
import 'node:events';
...
const gwas_mnv_get = defineEventHandler(async (event) => {
  const {
    // variant_type,
    variant_id,
    indicator_type,
  ...
  const [total, raw_result_cursor, indicator_type_stats] = await Promise.all([
    ...
    pageSizeNumber === -1 ? GwasMNV.find(query).sort(sortBy ? [[sortBy, Number(order)]] : []) : GwasMNV.find(query).sort(sortBy ? [[sortBy, Number(order)]] : []).skip(skip).limit(limit),
    // GwasMNV.aggregate([
    //     { $match: query },
...
export { gwas_mnv_get as default };
//# sourceMappingURL=gwas_mnv.get.mjs.map
