import { d as defineEventHandler, g as getQuery, b as GwasSNP } from '../../nitro/nitro.mjs';
import 'mongoose';
import 'node:http';
import 'node:https';
import 'node:events';
...
const gwas_snp_get = defineEventHandler(async (event) => {
  const {
    // variant_type,
    variant_id,
    indicator_type,
  ...
  const [total, raw_result_cursor, indicator_type_stats] = await Promise.all([
    GwasSNP.countDocuments(query),
    pageSizeNumber === -1 ? GwasSNP.find(query).sort(sortBy ? [[sortBy, Number(order)]] : []) : GwasSNP.find(query).sort(sortBy ? [[sortBy, Number(order)]] : []).skip(skip).limit(limit),
    // GwasSNP.aggregate([
    //     { $match: query },
...
export { gwas_snp_get as default };
//# sourceMappingURL=gwas_snp.get.mjs.map
