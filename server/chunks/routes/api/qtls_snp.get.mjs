import { d as defineEventHandler, g as getQuery, j as QtlSNP } from '../../nitro/nitro.mjs';
import 'mongoose';
import 'node:http';
import 'node:https';
import 'node:events';
...
const qtls_snp_get = defineEventHandler(async (event) => {
  const {
    variant_id,
    phenotype_type,
    phenotype_id,
  ...
  const [total, raw_result_cursor, phenotype_type_stats] = await Promise.all([
    // 查询总条数
    QtlSNP.countDocuments(query),
    // 查询主结果列表（带分页或不分页）
    pageSizeNumber === -1 ? QtlSNP.find(query).sort(sortBy ? [[sortBy, Number(order) || 1]] : []) : QtlSNP.find(query).sort(sortBy ? [[sortBy, Number(order) || 1]] : []).skip(skip).limit(limit),
...
export { qtls_snp_get as default };
//# sourceMappingURL=qtls_snp.get.mjs.map
