import { d as defineEventHandler, g as getQuery, f as GWASSchema } from '../../nitro/nitro.mjs';
import 'mongoose';
import 'node:http';
import 'node:https';
import 'node:events';
...
const gwas_get = defineEventHandler(async (event) => {
  const {
    variant_type,
    variant_id,
    indicator_type,
  ...
  const [total, raw_result_cursor, variant_type_stats, indicator_type_stats] = await Promise.all([
    ...
    pageSizeNumber === -1 ? GWASSchema.find(query).sort(sortBy ? [[sortBy, Number(order)]] : []) : GWASSchema.find(query).sort(sortBy ? [[sortBy, Number(order)]] : []).skip(skip).limit(limit),
...
export { gwas_get as default };
//# sourceMappingURL=gwas.get.mjs.map
